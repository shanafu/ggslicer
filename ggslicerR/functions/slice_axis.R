...
#' @importFrom magrittr %>%
#' @importFrom dplyr filter arrange mutate select bind_rows
#' @importFrom tidyselect all_of
#' @importFrom rlang sym
slice_axis <- function(file, slice_orientation, slice_orientation_coords = NULL) {
  ...
  voxel_coords$x <- as.numeric(world_coords_matrix[1,])
  voxel_coords$y <- as.numeric(world_coords_matrix[2,])
  voxel_coords$z <- as.numeric(world_coords_matrix[3,])
  
  axis_choice <- tolower(slice_orientation)
  filter_col_world <- NULL

  if (axis_choice %in% c("x", "sagittal", "1")) {
    filter_col_world <- "x"
  ...
  if (is.null(slice_orientation_coords)) {
    voxel_coords$slice_axis <- filter_col_world
    voxel_coords$slice_world <- voxel_coords[[filter_col_world]]

    # Reorder columns
    unfiltered_cols <- c("x", "y", "z", "i", "j", "k", "slice_axis", "slice_world")
    return(voxel_coords %>% select(all_of(unfiltered_cols)))
  }
  
  target_cols <- c("x", "y", "z", "i", "j", "k", "slice_axis", "slice_index", "slice_world")
  ...
  # Loop through each target coordinate in the order the user provided them, filtering and appending to the running data frame
  tolerance <- 1e-6
  df_slice_axis <- NULL

  for (loop_index in seq_along(target_slices)) {

    target_coord <- target_slices[loop_index]

    df_temp <- voxel_coords %>%
      filter(abs(!!sym(filter_col_world) - target_coord) < tolerance) %>%
      mutate(slice_index = loop_index,
             slice_world = !!sym(filter_col_world),
             slice_axis = filter_col_world)

    if (is.null(df_slice_axis)) {
      df_slice_axis <- df_temp
    } else {
      df_slice_axis <- bind_rows(df_slice_axis, df_temp)
    }
  }

  # Reorder columns
  df_slice_axis <- df_slice_axis %>%
